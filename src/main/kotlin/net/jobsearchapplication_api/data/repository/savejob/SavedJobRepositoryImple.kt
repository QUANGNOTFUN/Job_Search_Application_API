...
// data/repository/savedjob/SavedJobRepositoryImpl.kt
class SavedJobRepositoryImpl(
    private val savedJobService: SavedJobService
) : SavedJobRepository {
    override suspend fun getSavedJobs(userId: UUID): BaseResponse<Any> {
        return try {
    ...
    override suspend fun saveJob(userId: UUID, jobId: UUID): BaseResponse<Any> {
        return try {
            val result = savedJobService.saveJob(userId, jobId)
            if (result) {
                BaseResponse.SuccessResponse(
                    data = null,
